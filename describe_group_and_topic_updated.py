...
# 🔧 Update your Kafka broker address
    ...
    try:
        ...
        for topic in topic_metadata:
            ...
            for partition in topic['partitions']:
                ...
                if leader == -1:
                    status = "Unhealthy"
                elif len(isr) < len(replicas) and status != "Unhealthy":
                    status = "Degraded"

                print(f"    - Partition: {partition_id}")
                print(f"      Leader: {leader if leader != -1 else '❌ None'}")
            ...
            if status == "Healthy":
                ...
                print(f"  🟡 Topic Status: {status} (some replicas not in sync)")
            else:
                print(f"  🔴 Topic Status: {status} (missing partition leader)")
